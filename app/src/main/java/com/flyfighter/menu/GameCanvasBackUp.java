...
public class GameCanvasBackUp extends SurfaceView implements SurfaceHolder.Callback, Runnable {
    ...
    public static final int[][] playerData = new int[][]{{1, 106, 320, 5, 3, 3, 0, 3}, {2, 105, 320, 4, 3, 3, 0, 2}, {3, 105, 320, 6, 3, 3, 0, 1}};
    ...
    private static final int bu_type = 0;
    private static final int bu_picid = 1;
    private static final int bu_x = 2;
    private static final int bu_y = 3;
    private static final int bu_speedx = 4;
    private static final int bu_speedy = 5;
    ...
    public GameCanvasBackUp(Context context, int gSelectedPlayer) {
        super(context, null);
    ...
    private void gameInit() {
        ...
        this.gPlayer.setValue(data[0], (MainWindow.windowWidth - width) / 2, MainWindow.windowHeight + height, data[3], data[4], data[5], data[6], data[7]);
    ...
    private void stageInit() {
        ...
        this.gPlayer.y = MainWindow.windowHeight + ResInit.playerStateImage[5].getHeight() - 300;
    }
    ...
    private void dealPlayerState() {
        if (this.gScreenMove >= 10 && !this.gIsGameOver) {
            int wide = playerSize[this.gSelectedPlayer * 2];
            int high = playerSize[this.gSelectedPlayer * 2 + 1];
            if (this.gIsPlayerDestroyed) {
                this.gIsPlayerDestroyed = false;
                int idx = this.mallocBlastPT();
                if (idx >= 0) {
                    this.gExplode[idx][ex_type] = 8;
                    this.gExplode[idx][ex_x] = (this.gPlayer.x + (wide >> 1) - (explodeSize[14] >> 1));
                    this.gExplode[idx][ex_y] = (this.gPlayer.y + (high >> 1) - (explodeSize[15] >> 1));
                ...
                for (ii = 0; ii < 100; ++ii) {
                    for (ii = 0; ii < 6; ++ii) {
                        this.gBullet[ii][ii] = 0;
                    }
                }

                for (ii = 0; ii < 4; ++ii) {
                    for (ii = 0; ii < 7; ++ii) {
                        this.gMissile[ii][ii] = 0;
                    }
                }
                ...
                if (this.gPlayer.life <= 0) {
                    ...
                    int life = this.gPlayer.life;
                    this.gPlayer.setValue(playerData[this.gSelectedPlayer][0], playerData[this.gSelectedPlayer][1], playerData[this.gSelectedPlayer][2], playerData[this.gSelectedPlayer][3], playerData[this.gSelectedPlayer][4], playerData[this.gSelectedPlayer][5], playerData[this.gSelectedPlayer][6], playerData[this.gSelectedPlayer][7]);
                    this.gPlayer.life = life;

                    if (2 == this.gGameSecretNum) {
                        this.gPlayer.bomb = 6;
                    }

                    for (ii = 0; ii < 6; ++ii) {
                        this.gBombType[ii] = this.gPlayer.type;
                    }
                    this.gPlayerInit = 1;
                }
            } else {
                if ((this.gScreenMove & 1) == 0) {
                    this.gPlayer.picId = 0;
                } else {
                    this.gPlayer.picId = 1;
                }

                PlayerPlane var10000;
                if ((this.gPlayerInit == 0 || this.gPlayerInit > 20) && !this.gIsMissionComplete) {
                    int playerspd = this.gPlayer.speed;
                    if (this.gKey_State[2]) {
                        if (this.gPlayer.x - playerspd > -(wide >> 1)) {
                            var10000 = this.gPlayer;
                            var10000.x = (var10000.x - playerspd);
                        }

                        this.gPlayer.picId = 2;
                    } else if (this.gKey_State[3]) {
                        if (this.gPlayer.x + playerspd < MainWindow.windowWidth - (wide >> 1)) {
                            var10000 = this.gPlayer;
                            var10000.x = (var10000.x + playerspd);
                        }
                        this.gPlayer.picId = 3;
                    }

                    if (this.gKey_State[1]) {
                        if (this.gPlayer.y - playerspd > -(high >> 1)) {
                            var10000 = this.gPlayer;
                            var10000.y = (var10000.y - playerspd);
                        }
                    } else if (this.gKey_State[0] && this.gPlayer.y + playerspd < MainWindow.windowHeight - (high >> 1)) {
                        var10000 = this.gPlayer;
                        var10000.y = (var10000.y + playerspd);
                    }

                    if (this.gKey_State[4] && this.gFireDelay > 1) {
                        this.gFireDelay = 0;
                        this.makePlayerBullet();
                        if ((this.gScreenMove & 17) < 2) {
                            this.makeMissile();
                        }
                        this.gIsPlayerFire = true;
                    }

                    ++this.gFireDelay;
                    if (this.gKey_State[5] && !this.gIsBombing) {
                        this.makeBomb();
                    }
                }

                this.gKey_State[5] = false;
                if (this.gPlayerInit > 80) {
                    this.gPlayerInit = 0;
                } else if (this.gPlayerInit != 0) {
                    ++this.gPlayerInit;
                    if (this.gPlayerInit < 25) {
                        var10000 = this.gPlayer;
                        var10000.y = (var10000.y - 2);
                    }
                }
            }
        }
    ...
    private void makePlayerBullet() {
        //this.playSound(this.gSelectedPlayer + 5);
        int bulletNum = (this.gPlayer.power - 1);

        int bulletNumTemp = bulletNum;

        int offSet;
        for (offSet = playerBulletFloorNum[this.gSelectedPlayer][5]; bulletNumTemp-- > 0; offSet = (offSet + playerBulletFloorNum[this.gSelectedPlayer][bulletNumTemp])) {
        }

        int[] var6;
        for (bulletNumTemp = playerBulletFloorNum[this.gSelectedPlayer][bulletNum]; bulletNumTemp-- > 0; var6[3] += this.gPlayer.y) {
            int idx = this.mallocBullet();
            if (idx < 0) {
                return;
            }

            for (int ii = 0; ii < 5; ++ii) {
                this.gBullet[idx][ii + 1] = playerBulletData[offSet][ii];
            }

            ++offSet;
            this.gBullet[idx][0] = this.gPlayer.type;
            var6 = this.gBullet[idx];
            var6[2] += this.gPlayer.x;
            var6 = this.gBullet[idx];
        }
    ...
    private void dealBullet() {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (type != 0) {
                var10000 = this.gBullet[i];
                var10000[2] += this.gBullet[i][4];
                var10000[3] += this.gBullet[i][5];

                wide = bulletSize[(type - 1) * 2];
                high = bulletSize[(type - 1) * 2 + 1];

                if (this.gBullet[i][2] + this.gBullet[i][4] > -wide && this.gBullet[i][2] + this.gBullet[i][4] < MainWindow.windowWidth && this.gBullet[i][3] + this.gBullet[i][5] > -high && this.gBullet[i][3] + this.gBullet[i][5] < MainWindow.windowHeight) {
                    if (type <= 3) {
                        for (int j = 0; j < 5 && !this.checkHitEnemy(wide, high, i, j); ++j) {
                        }
                    } else {
                        this.checkHitPlayer(wide, high, i);
                        if ((this.gScreenMove & 1) != 0) {
                            ++this.gBullet[i][1];
                        }

                        if (this.gBullet[i][1] >= bulletPic[type - 1]) {
                            this.gBullet[i][1] = 0;
                        }
                    }
                } else {
    ...
    private void checkHitPlayer(int bullet_wide, int bullet_high, int i) {
        int player_x = (this.gPlayer.x + 6);
        int player_y = (this.gPlayer.y + 7);
        int player_wide = (playerSize[this.gSelectedPlayer * 2] - 10);
        int player_high = (playerSize[this.gSelectedPlayer * 2 + 1] - 12);
        if (this.checkIfHit(this.gBullet[i][2], this.gBullet[i][3], bullet_wide, bullet_high, player_x, player_y, player_wide, player_high)) {
            if (6 != this.gGameSecretNum && this.gPlayerInit == 0 && !this.gIsBombing) {
    ...
    private boolean checkHitEnemy(int bulletWide, int bulletHigh, int i, int j) {
        int type = this.gEnemy[j].type;
        if (type == 0) {
            return false;
        } else {
            int bullet_x = this.gBullet[i][2];
            int bullet_y = this.gBullet[i][3];
            int enemy_x = this.gEnemy[j].x;
            int enemy_y = this.gEnemy[j].y;
            int enemy_wide;
            int enemy_high;
            if (type > 200) {
                enemy_wide = enemySize[(47 + this.gCorrespondMission - 2) * 2];
                enemy_high = enemySize[(47 + this.gCorrespondMission - 2) * 2 + 1];
            } else {
                enemy_wide = enemySize[(type - 1) * 2];
                enemy_high = enemySize[(type - 1) * 2 + 1];
            }

            if (this.checkIfHit(bullet_x, bullet_y, bulletWide, bulletHigh, enemy_x, enemy_y, enemy_wide, enemy_high)) {
                if (type > 200) {
                    this.gEnemy[j].picId = (enemyPic[47 + this.gCorrespondMission - 2] - 1);
                } else {
                    this.gEnemy[j].picId = (enemyPic[this.gEnemy[j].type - 1] - 1);
                }

                int idx = this.mallocBlastPT();
                if (idx >= 0) {
                    this.gExplode[idx][ex_type] = (1 + this.getRand(2));
                    this.gExplode[idx][ex_x] = bullet_x;
                    this.gExplode[idx][ex_y] = (bullet_y - 2);
                    this.gExplode[idx][ex_picid] = 0;
                }

                EnemyPlane var10000 = this.gEnemy[j];
                var10000.health = (var10000.health - (playerBulletFloorPower[this.gSelectedPlayer][this.gPlayer.power - 1] - 1));
                this.destroyBullet(i);
                this.gGameScore += 28;
                return true;
            } else {
    ...
    private void dealEnemyState() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if ((this.gEnemy[i]).offset < 300) {
                ...
                //两种情况下方向改变,offset固定触发或者是碰撞到边缘触发
                if ((this.gEnemy[i]).offset > 0 && this.gScreenMove % ((this.gEnemy[i]).offset * 3) == 0 && getRand(2) == 1) {
                ...
                x = (x + (this.gEnemy[i]).speedX);
                y = (y + (this.gEnemy[i]).speedY);
                if (enemyOffset != 0 && this.gScreenMove % enemyOffset == 0) {
    ...
    private void makeEnemyBullet(int i) {
        ...
        if ((this.gEnemy[i]).y <= 0 || (this.gEnemy[i]).y >= MainWindow.windowHeight - 50 || (this.gEnemy[i]).x <= 50 - wide ||
                (this.gEnemy[i]).x >= MainWindow.windowWidth - 50) {
            return;
        }
        while (true) {
            if (this.gShoot_SL[i]) {
                if (this.gScreenMove % 5 == 0) {
                    shootpoint[0] = ((enemySize[((this.gEnemy[i]).type - 1) * 2] >> 1) - 22);
                    shootpoint[1] = (enemySize[((this.gEnemy[i]).type - 1) * 2 + 1] - 22);
        ...
        //受到帧率的过高导致子弹变多,随机变大一点
        int ranVal = this.getRand(6 - this.gGameDifficulty);
        if (ranVal % 3 == 0) {
    ...
    private void makeBulletToPlayer(int i, int bulletNum, int x, int y, int type) {
        ...
        if (bulletNum >= 3) {
            ...
            this.gBullet[idx][4] = bulletSpeedToPlayer[direction * 6 - 3];
            this.gBullet[idx][5] = bulletSpeedToPlayer[direction * 6 - 2];
        }
...
//        if (this.gCurrentMissileMax > 0) {
//            for (i = 0; i < 4; ++i) {
//                if (this.gMissile[i][0] != 0) {
//                    x = this.gMissile[i][2];
//                    y = this.gMissile[i][3];
//                    int wide;
//                    int high;
//                    if (this.gMissile[i][0] == 2) {
//                        short xpic = this.getMissilePicIndex(i);
//                        wide = 8;
//                        high = 8;
//                        if (x > -wide && x < MainWindow.windowWidth && y > -high && y < MainWindow.windowWidth) {
//                            this.setRect((xpic * wide), 0, wide, high);
//                            this.gameDrawImage(ResInit.bulletImage[58], x, y, true, g);
//                        }
//                    } else {
//                        wide = 6;
//                        high = 28;
//                        if (x > -wide && x < MainWindow.windowWidth && y > -high && y < MainWindow.windowWidth) {
//                            this.setRect((short) ((this.gScreenMove & 1) * wide),  0, wide, high);
//                            this.gameDrawImage(ResInit.bulletImage[57], x, y, true, g);
//                        }
//                    }
//                }
//            }
//        }
    ...
    private void showActiveBomb(Canvas canvas) {
        ...
        for (int i = 0; i < this.gPlayer.bomb; i = (i + 1)) {
            int x = (27 + i * 45);
    ...
    private void showActiveLife(Canvas canvas) {
        for (int i = 0; i < this.gPlayer.life - 1; i = (i + 1)) {
            int x = (27 + i * 45);
